    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	count += snprintf(buf + count, PAGE_SIZE, "Resolution:(%d,%d)~(%d,%d)\n",
	                  ...
	                  0, 0, pdata->max_x, pdata->max_y);

	count += snprintf(buf + count, PAGE_SIZE, "Max Touchs:%d\n",
	                  ts_data->max_touch_number);
...
/************************************************************************/
static ssize_t fts_pocket_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	u8 val = 0;
	...
	count = snprintf(buf, PAGE_SIZE, "Pocket Mode:%s\n",
					 val ? "On" : "Off");
	count += snprintf(buf + count, PAGE_SIZE, "Reg(0xB2)=%d\n", val);
	if ((val) != ts_data->pdata->pocket_mode)
		ts_data->pdata->pocket_mode = val;
...
static ssize_t fts_pocket_store(
	struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct fts_ts_data *ts_data = fts_data;
	...
	mutex_lock(&ts_data->pdata->enable_mutex);
	if (FTS_SYSFS_ECHO_ON(buf)) {
		FTS_DEBUG("enable pocket");
		ts_data->pdata->pocket_mode = ENABLE;
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		FTS_DEBUG("disable pocket");
		ts_data->pdata->pocket_mode = DISABLE;
	}
...
static DEVICE_ATTR(fts_pocket_mode, 0644, fts_pocket_show, fts_pocket_store);
...
static ssize_t fts_fod_mode_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	u8 val = 0;
	...
	mutex_lock(&ts_data->pdata->enable_mutex);
	fts_read_reg(FTS_REG_FOD_MODE, &val);
	count += snprintf(buf + count, PAGE_SIZE, "Reg(0xCF)=%x\n", val);
...
static ssize_t fts_fod_mode_store(
	struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct fts_ts_data *ts_data = fts_data;
	mutex_lock(&ts_data->pdata->enable_mutex);
	ts_data->fod_mode = shex_to_int(buf, 2);
	FTS_INFO("fod_mode = %x\n", ts_data->fod_mode);
...
static DEVICE_ATTR(fts_fod_mode, 0644, fts_fod_mode_show, fts_fod_mode_store);

static ssize_t fts_fod_rect_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	u8 fod_rect[8] = {0};
	...
	fts_read(fod_rect, 1, fod_rect, 8);
	count += snprintf(buf + count, PAGE_SIZE, "rect = ");
	for (i = 0; i < 8; i++) {
		count += snprintf(buf + count, PAGE_SIZE, "%x", fod_rect[i]);
	}
	count += snprintf(buf + count, PAGE_SIZE, "\n");
	mutex_unlock(&ts_data->pdata->enable_mutex);
...
static ssize_t fts_fod_rect_store(
	struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct fts_ts_data *ts_data = fts_data;
	...
	mutex_lock(&ts_data->pdata->enable_mutex);
//	printk("[FTS_TS] set fod rect:");
	for (i = 0; i < 8; i++) {
		fod_rect[i + 1] = shex_to_int(buf + (i * 3), 2);
//		printk("%x,", fod_rect[i + 1]);
	}
//	printk("\n");
	fod_rect[0] = FTS_REG_FOD_RECT;
...
static DEVICE_ATTR(fts_fod_rect, 0644, fts_fod_rect_show, fts_fod_rect_store);
