						...
						const apk_sign_key_t *keys)
{
	...
	while (loop_count++ < 10) {
		...
		if (id == 0x7109871au) {
			v2_signing_blocks++;
			v2_signing_valid =
				check_block(fp, &size4, &pos, &offset, keys);
...
#endif
	return check_v2_signature(path, apk_sign_keys);
}
