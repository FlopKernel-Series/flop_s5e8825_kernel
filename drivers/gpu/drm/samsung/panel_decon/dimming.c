...
s64 decon_disp_pow(s64 num, u32 digits)
{
	s64 res = num;
	u32 i;
...
s64 disp_round(s64 num, u32 digits)
{
	...
	tnum = num * sign;
	tnum *= decon_disp_pow(10, digits + 1);
	tnum += 5;
	do_div(tnum, (u32)decon_disp_pow(10, digits + 1));
...
static s64 scale_down_round(s64 num, u32 digits)
{
	...
	rem *= decon_disp_pow(10, digits + 1);
	rem >>= BIT_SHIFT;
	rem += 5;
	do_div(rem, (u32)decon_disp_pow(10, digits + 1));
...
static s64 scale_down_rem(s64 num, u32 digits)
{
	...
	if (rem >= (u64)decon_disp_pow(10, digits))
		rem -= (u64)decon_disp_pow(10, digits);
...
s64 decon_disp_div64(s64 num, s64 den)
{
#if BIT_SHIFT > 26
...
EXPORT_SYMBOL(decon_disp_div64);

#ifdef DIMMING_CALC_PRECISE
s64 decon_disp_div64_round(s64 num, s64 den, u32 digits)
{
	int sign_num = (num < 0 ? -1 : 1);
	int sign_den = (den < 0 ? -1 : 1);
	int sign = sign_num * sign_den;
	...
	rem *= decon_disp_pow(10, digits + 2);
	do_div(rem, (u32)tden);
	 ...
	 */
	rem += 5;
	do_div(rem, (u32)decon_disp_pow(10, digits + 2));
		...
		int v, enum color c)
{
	...
	if (v == TP_VT) {
		...
		res = vsrc - decon_disp_div64(num, den);
	} else if (v == TP_V0) {
		s32 offset = tp[v].center[c] + tp[v].offset[c];
		if ((offset > 0xF) || (offset < 0x0)) {
			panel_warn("ivt (%d) out of range(0x0 ~ 0xF) replace to %s\n",
...
#ifdef DIMMING_CALC_PRECISE
	/* To get precise value, use decon_disp_div64_round() function */
	num -= den * (tp[v].numerator);
	num = disp_round(num, 7) - den * (s64)tp[v].offset[c];
	res = decon_disp_div64_round(num, den, 2);
...
s64 interpolation_round(s64 from, s64 to, int cur_step, int total_step)
{
	...
	den = (s64)total_step;
	num = from + sign * decon_disp_div64(num, den);

	return num;
}

s64 decon_disp_interpolation64(s64 from, s64 to, int cur_step, int total_step)
{
	s64 num, den;
	...
	den = (s64)total_step;
	num = from + decon_disp_div64(num, den);

	return num;
}
EXPORT_SYMBOL(decon_disp_interpolation64);
		...
		s32 (*out)[MAX_COLOR], int nr_tp, int cur_step, int total_step)
{
	...
	for (i = 0; i < nr_tp; i++) {
		for_each_color(c) {
			out[i][c] =
				decon_disp_interpolation64(from[i][c], to[i][c], cur_step, total_step);
#ifdef DEBUG_DIMMING
...
int decon_gamma_table_interpolation_round(s32 (*from)[MAX_COLOR], s32 (*to)[MAX_COLOR],
		s32 (*out)[MAX_COLOR], int nr_tp, int cur_step, int total_step)
{
	int i, c;
...
EXPORT_SYMBOL(decon_gamma_table_interpolation_round);
...
static int generate_gray_scale(struct dimming_info *dim_info)
{
	...
	for (i = 0, iv = (TP_V0 > 0 ? TP_V0 : TP_VT);
		...
		for_each_color(c) {
			...
			vout = decon_disp_interpolation64(v_lower, v_upper, cur_step, total_step);
#ifdef DEBUG_DIMMING
...
void decon_get_dimming_gamma(struct dimming_info *dim_info, u32 luminance, u8 *output,
		void (*copy)(u8 *output, u32 value, u32 index, u32 color))
{
...
EXPORT_SYMBOL(decon_get_dimming_gamma);
...
int decon_init_dimming_info(struct dimming_info *dim_info, struct dimming_init_info *src)
{
	int ilum;
...
EXPORT_SYMBOL(decon_init_dimming_info);

int decon_init_dimming_mtp(struct dimming_info *dim_info, s32 (*mtp)[MAX_COLOR])
{
	int i, c;
...
EXPORT_SYMBOL(decon_init_dimming_mtp);
...
int decon_process_dimming(struct dimming_info *dim_info)
{
	int i, c, v;
	struct tp *tp;
	s64 VREGOUT;
...
EXPORT_SYMBOL(decon_process_dimming);
