...
#define AOR_TO_RATIO(aor, vtotal) \
	(((aor) * usdm_disp_pow(10, 5) / (vtotal) + 5 * usdm_disp_pow(10, 0)) / usdm_disp_pow(10, 1))

#define VIRTUAL_BASE_LUMINANCE(lum, aor_ratio) \
	((lum) * usdm_disp_pow(10, 6) / (usdm_disp_pow(10, 4) - (aor_ratio)))
...
int usdm_get_actual_brightness_index(struct panel_bl_device *panel_bl, int brightness);
int usdm_get_subdev_actual_brightness_index(struct panel_bl_device *panel_bl,
...
bool usdm_is_hbm_brightness(struct panel_bl_device *panel_bl, int brightness);
...
int usdm_panel_bl_aor_interpolation_2(struct panel_bl_device *panel_bl,
		int id, u8 (*aor_tbl)[2]);
int usdm_panel_bl_irc_interpolation(struct panel_bl_device *panel_bl, int id,
